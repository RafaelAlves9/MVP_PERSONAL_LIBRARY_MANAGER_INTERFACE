...
export function toBook(raw: unknown): Book {
  ...
  return {
    externalId: data.external_id ?? data.key ?? '',
    title: data.title,
  ...
  const container = paginatedSchema.safeParse(raw).success
    ...
    : (() => {
        ...
        return {
          ...
          totalPages: (fallback.totalPages as number) ?? (fallback.total_pages as number) ?? 1,
          total: (fallback.total as number) ?? (fallback.total_items as number) ?? items.length,
          limit: (fallback.limit as number) ?? (fallback.per_page as number) ?? 10,
        }
      })()
